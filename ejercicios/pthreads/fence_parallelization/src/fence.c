...
void find_maximum_perimeter(const size_t thread_num,
  ...
  for (size_t top_left_row = thread_num; top_left_row < shared_data->rows - 1;
    top_left_row += shared_data->thread_count) {
    // Repeat top left column for each column of terrain except last one
    for (size_t top_left_column = 0; top_left_column < shared_data->columns - 1;
      ++top_left_column) {
      // Create local perimeter as the result of
      // finding maximum perimeter that can be formed from row and column
      coordinate_t local_bottom_right =  find_maximum_local_perimeter
        ...
        size_t local_perimeter = calculate_perimeter
        ((coordinate_t){top_left_row, top_left_column}, local_bottom_right);
...
// Find maximum perimeter that can be
// formed from (top left row, top left column):
coordinate_t find_maximum_local_perimeter(const size_t top_left_row,
  ...
  for (size_t bottom_right_row = top_left_row + 1;
    ...
    for (size_t bottom_right_column = top_left_column + 1;
      bottom_right_column < shared_data->columns; ++bottom_right_column) {
      // If can form rectangle from top left cell to bottom right cell
      if (can_form_rectangle(top_left_row, top_left_column,
        bottom_right_row, bottom_right_column, shared_data)) {
        // Create local perimeter as result of calculate perimeter
        size_t perimeter =
        calculate_perimeter((coordinate_t){top_left_row, top_left_column},
...
bool can_form_rectangle(const size_t top_left_row, const size_t top_left_column,
  const size_t bottom_right_row,
  const size_t bottom_right_column,
  ...
  for (size_t row = top_left_row + 1; row <= bottom_right_row - 1; ++row) {
    ...
    if (shared_data->terrain[row][top_left_column] ==
    'x' ||  shared_data->terrain[row][bottom_right_column] == 'x') {
      // Return false
      return false;
    }
  }
...
size_t calculate_perimeter(const coordinate_t top_left,
  ...
  // Return 2 * (bottom right column - top left column)
  // + 2 * (bottom right row - top left row)
  return 2 * (bottom_right.column - top_left.column) +
    2 * (bottom_right.row - top_left.row);
}
